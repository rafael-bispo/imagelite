...
class AuthService{
    ...
    initSession(token: AccessToken){
        if(token.accessToken){
            const decodeToken: any = jwt(token.accessToken);
            ...
            this.setUserSesion(userSessionToken);
        }    
    }

    setUserSesion(userSessionToken: UserSessionToken){
        try{
    ...
    //remove da sessão o usuário tirando do localstorge o token
