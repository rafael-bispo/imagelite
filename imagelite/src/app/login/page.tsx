...
export default function Login(){
    ...
    async function onSubmit(values : LoginForm) {
        if(!newUserState){
            ...
            try{
                //solicita o token ao back
                const accessToken: AccessToken = await auth.authenticate(credentials);

                //inicia a sessão
                auth.initSession(accessToken);

                //verificando se a sessão está válida
                console.log("Sessão está válida", auth.isSessionValid());

                //levao usuário pra página de galeria
                router.push("/galeria");
            } catch(error: any){
            ...
            try{
                ...
                notification.notify("Seccess on saving new user!", "success");
                resetForm();
                    ...
                    <form onSubmit={handleSubmit} className="space-y-2">
                        ...
                        <div>
                            <RenderIf condition={!newUserState}>
                                ...
                                <Button 
                                    type="button" 
                                    style="bg-blue-600 hover:bg-blue-400 mx-2" 
                                    label="Sign Up" 
